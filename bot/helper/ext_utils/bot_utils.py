...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            msg += f"\n\n<b>⌈➳📂 File Name : {escape(str(download.name()))}</b>"
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>⌈➳📊 Progress : {get_readable_file_size(download.processed_bytes())} of {download.size()}</b>"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>⌈➳📊 Progress : {get_readable_file_size(download.processed_bytes())} of {download.size()}</b>"
                else:
                    msg += f"\n<b>⌈➳📊 Progress : {get_readable_file_size(download.processed_bytes())} of {download.size()}</b>"
                msg += f"\n<b>⌈➳🚀 Speed : {download.speed()}</b>\n<b>⌈➳🕰️ Waiting Time : {download.eta()}</b>"
                msg += f"\n<b>⌈➳⌚ Elapsed : {get_readable_time(time() - download.message.date.timestamp())}</b>"
                msg += f'\n<b>⌈➳🗣️ User : <a href="https://t.me/c/{str(download.message.chat.id)[4:]}/{download.message.message_id}">{download.message.from_user.first_name}</a></b>'
                ...
                msg += f"\n<b>⌈➳🚈 Engine : qBittorrent v4.4.2</b>"
                msg += f"\n<b>⌈➳🚀 Speed : {get_readable_file_size(download.torrent_info().upspeed)}/s</b>"
                msg += f" | <b>⌈➳📊 Progress : {get_readable_file_size(download.torrent_info().uploaded)}</b>"
        ...
        bmsg += f"<b> | ⌈➳🕰️ UPTM :</b> {get_readable_time(time() - botStartTime)}"
        dlspeed_bytes = 0
        upspeed_bytes = 0
        ...
        bmsg += f"\n<b>⌈➳🔻 DN :</b> {get_readable_file_size(dlspeed_bytes)}/s<b> | ⌈➳🔺 UP : {get_readable_file_size(upspeed_bytes)}/s</b>"
