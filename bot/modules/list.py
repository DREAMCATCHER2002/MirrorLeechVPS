...
def list_buttons(update, context):
    ...
    if len(context.args) == 0:
        return sendMessage('<b>📨 Send a search key along with command</b>', context.bot, update.message)
    buttons = button_build.ButtonMaker()
    buttons.sbutton("📂 Folders 📂", f"types {user_id} folders")
    buttons.sbutton("🗂️ Files 🗂️", f"types {user_id} files")
    buttons.sbutton("📂 Both 🗂️", f"types {user_id} both")
